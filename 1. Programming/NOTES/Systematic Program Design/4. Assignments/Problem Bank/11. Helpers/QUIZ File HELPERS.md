...
;; ======================================================================
;; Constants
(define COOKIES 0) ; should be a pic of cookies, doesnt copy over
...
(define (pyramid n i)
  (cond [(zero? n) empty-image]
        [else
         (above(pyramid (sub1 n) i)(draw-cookies n i) )]))

;;Natural Image -> Image
;;Draws Template amount of cookies needed
(define (draw-cookies n i)
  (cond [(zero? n) empty-image]
        [else (beside i (draw-cookies (sub1 n )i))]))
...
(define (sink lob)
  (cond [(empty? lob) empty]
        [else
         (check (first lob)(sink (rest lob)))]))
;;108
;; (check s (sink (cons b (s e))))
;; (check s (check b (sink (cons s e))))
;; (check s (check b (check s (sink e))))
;; (check s (check b (check s (check e))))
;; (check s (check b (check s empty)))
;; (check s (check b (cons s empty)))
;; (check s (cons b (cons s empty)))
;; (cons b (cons s (cons s empty)))
(define (check b lob)
  (cond [(empty? lob) (cons b empty)]
        [(and(string=? b "solid")(string=? (first lob) "bubble"))
